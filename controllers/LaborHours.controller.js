const LaborHours = require("../models/LaborHours.model")
...
// }
const createRecord = async ({id,fiscalWeek,culinary,service,togo,host,bartender,category}) => {
    try{
        const laborHours = new LaborHours({
            id,
            fiscalWeek,
            culinary,
            service,
        ...
        await laborHours.save() //saves states into database
        return laborHours._id
}
catch(err){
    console.error(err);
...
const findAllRecords = async (limit =0) => {
    const record = await LaborHours.find();
    return record
}
...
const findAllRecordById = async _id =>{
    try {
        const record = await LaborHours.findById({_id});
        if(LaborHours === null){
...
//Update Record
const updateRecord = async({_id,fiscalWeek,culinary,service,togo,host,bartender,category})=>{
    try {
        const updates={_id,fiscalWeek,culinary,service,togo,host,bartender,category};
        const updatedRecord = await LaborHours.findByIdAndUpdate({_id}, updates,{new:true});
