const LaborStats = require("../models/LaborStats.model")
...
const createRecord = async ({id,fiscalWeek,culinary,service,togo,host,bartender,category}) => {
    try{
        const laborStats = new LaborStats({
            id,
            fiscalWeek,
            culinary,
            service,
        ...
        await laborStats.save() //saves states into database
        return laborStats._id
}
catch(err){
    console.error(err);
...
const findAllRecords = async (limit =0) => {
    const record = await LaborStats.find();
    return record
}
...
const findAllRecordById = async _id =>{
    try {
        const record = await LaborStats.findById({_id});
        if(LaborStats === null){
...
const updateRecord = async({_id,fiscalWeek,culinary,service,togo,host,bartender,category})=>{
    try {
        ...
        const updatedRecord = await LaborStats.findByIdAndUpdate({_id}, updates,{new:true});
